...
#     getMemberships
    ...
    # Description: Creates a new membership in a `goal`. `Teams` or `users` can be a member
    #   of `goals`.
    #
    ...
    # Description: A specific, existing membership can be deleted by making a `DELETE` request
    ...
    # Description: Returns compact `goal_membership` records. The possible types for `parent` in this request are `goal`. An additional member (user GID or team GID) can be passed in to filter to a specific membership.
    #
    # Parameters:
    #     member:
...
#     updateProject
    ...
    # Description: Get an object that holds task count fields. **All fields are excluded by default**. You must [opt in](/docs/inputoutput-options) using `opt_fields` to get any information from this endpoint.
    #
