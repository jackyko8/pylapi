...
# def dotted_paths(attrs, prefix="$", include_branches=False):
#     paths = []
#     if type(attrs) == dict:
#         for attr_name in attrs:
#             attr_path = prefix + "." + attr_name
#             sub_path = dotted_paths(attrs[attr_name], prefix=attr_path, include_branches=include_branches)
#             if sub_path == []:
#                 # Leaf node
...
# def dotted_update(attrs:dict, src:dict):
#     # For each leaf element in src, update attrs
#     for src_path in dotted_paths(src):
#         # logger.debug(f"new_value=dotted_dict(src, src_path): src={src}; src_path={src_path}")
#         new_value = dotted_dict(src, src_path)
#         # logger.debug(f"set attrs={attrs} on {src_path} to {new_value} ({type(new_value)})")
#         dotted_dict(
#             attrs,
#             src_path,
#             value=new_value,
